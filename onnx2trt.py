...
def preprocess(img_path):
    ...
    input_h = 288
    input_w = 800
    image = cv2.resize(img_src, (input_w, input_h), interpolation=cv2.INTER_LINEAR)
...
def main(args):
    ...
    builder.create_network(args.onnx, input_shapes=dict(
        input=dict(min_shape=[1, 3, 288, 800],
                   opt_shape=[1, 3, 288, 800],
                   max_shape=[1, 3, 288, 800])))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--onnx", default="./UltraFastLaneDetection.onnx", help="The input ONNX model file to load")
    parser.add_argument("--engine", default="./UltraFastLaneDetection_tusimple_int8.trt", help="The output path for the TRT engine")
    parser.add_argument(
        "-p",
        "--precision",
        default="int8",
